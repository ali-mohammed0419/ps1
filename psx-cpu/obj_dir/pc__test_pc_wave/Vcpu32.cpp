...
std::unique_ptr<VerilatedTraceConfig> Vcpu32::traceConfig() const {
...
void Vcpu32___024root__trace_decl_types(VerilatedVcd* tracep);

void Vcpu32___024root__trace_init_top(Vcpu32___024root* vlSelf, VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vcpu32___024root__trace_decl_types(tracep);
    Vcpu32___024root__trace_init_top(vlSelf, tracep);
    tracep->popPrefix();
}

VL_ATTR_COLD void Vcpu32___024root__trace_register(Vcpu32___024root* vlSelf, VerilatedVcd* tracep);

VL_ATTR_COLD void Vcpu32::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vcpu32::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace-vcd with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP), name(), false, 4);
    Vcpu32___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
